...
export const Project = (props) => {
    return (
        <a href={props.link} className="transition-transform drop-shadow-sm hover:text-orange border border-gray-900 project min-[400px]:w-96 sm:w-full relative" target="_blank">
            ...
            <span className="absolute top-0 left-0 project-tags text-light font-poppins text-base lg:text-md xl:text-xl z-50 bg-dark bg-opacity-75 p-2 opacity-100 md:opacity-0 transition-opacity">{props.tags}</span>
        </a>
    );
}